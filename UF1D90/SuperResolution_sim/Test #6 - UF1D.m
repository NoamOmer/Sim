...
% Add noise
if (0)
	noise = transpose(rand(1,length(Sig)) + i*rand(1,length(Sig)));  % [ 0.0 ..  1.0]
	noise = noise - 0.5;                                             % [-0.5 .. +0.5]
	noise = 0.1 * noise * max(abs(Sig));                             % Scale to signal values
	figure; plot(abs(Sig)      ,'b.-'); hold on;
	        plot(abs(noise)    ,'r.-');
	        plot(abs(Sig+noise),'k.-');
	figure; plot(real(Sig)     ,'b.-'); hold on;
	        plot(real(noise)   ,'r.-');
	Sig2 = Sig + noise;
else
	Sig2 = Sig;
...
% The maximal resolution increase is equal or smaller than
% the number of acquisition points per effective-pixel
Overlapping_Factor = (Ta/dta) / (Lz/dZa);
SR_Factor = 0.9 * Overlapping_Factor; % 2.5;
disp(sprintf('Trying to increase image resolution %3.3f [mm] by a factor of %3.1f',dZa,SR_Factor));
disp(sprintf('(overlapping factor is %3.2f)',Overlapping_Factor));
SR_M = increase_1D_image_resolution(Sig2(end:-1:1),alpha0,alpha1,alpha2,Ga(3),Ta,Lz,dZa,dZa/SR_Factor);
...
title(sprintf('Initial, Reconstructed & Super-Res Magnetization (SRF=%3.2f)',SR_Factor));
xlabel('z [cm]');  ylabel('Magnetization');
