...
% Ver_7: Left-Hand-Rule --> Right-Hand-Rule
%
%  Output parameters:
%  ------------------
...
% Excitation gradient
% -------------------
%alpha2 = sign(Zfa-Zia)*1/(2*(dZa^2));                 % [1/cm^2]
alpha2 = 1/(2*(dZa^2));                                % [1/cm^2]
alpha1 = -(Ta/(Zfa-Zia))*(DNu0(end)) - 2*alpha2*Zfe_;  % [1/cm]   We assume that Zfe=Zia
alpha0 = 0;                                            % [none]

% Calculate the excitation gradient: Ge
Ge = (1/(gammaHz*Tp)) * (2*alpha2*Zie_ + alpha1 - (Tp+Ta)*dDNu0_dz(1) + (Ta/(Zie_-Zfe_))*DNu0(1) );
...
if (DEBUG_FLAG >= 2)
	...
	subplot(2,3,5); plot(ze_of_OmegaE(2:end),1E-3*1./dze_dOmegaE_app,'.-');
	xlabel('z-axis [cm]');            ylabel('[kHz/cm]');
	title('d\omega_E/dz (app)');      grid;    set_gca;

	subplot(2,3,6); plot(ze_of_OmegaE(2:end),1E+3*dze_dOmegaE_app,'.-'); 
	xlabel('z-axis [cm]');            ylabel('[cm/kHz]');
...
% Square part of the excitation phase
P_DNu0_ = P_DNu0;
P_DNu0_(end-1) = 0;
P_INT_DNu0_ = polyint(P_DNu0_);
DNu0_       = polyval(P_DNu0_     ,z_axis_);     % [Hz]     Polynomial approximation for B0 inhomogeneity
INT_DNu0_   = polyval(P_INT_DNu0_ ,z_axis_);     % [Hz*cm]  spatial integral
Phi_e_sq= alpha2*(z_axis_.^2)                  + alpha0 - (Ta/(Zfa-Zia)) * (DNu0_.*(z_axis_ - Zia) - 2*INT_DNu0_);
