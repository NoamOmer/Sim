    ...
    mclOutputHandlerFcn print_handler)
{
  ...
  if (!mclmcrInitialize())
    return false;
  if (!GetModuleFileName(GetModuleHandle("liba_HT"), path_to_dll, _MAX_PATH))
    return false;
    {
...
{
  return liba_HTInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
  return mclFeval(_mcr_inst, "HT", nlhs, plhs, nrhs, prhs);
}

LIB_liba_HT_C_API 
bool MW_CALL_CONV mlfHT(int nargout, mxArray** y, mxArray* x)
{
  return mclMlfFeval(_mcr_inst, "HT", nargout, 1, 1, y, x);
}
