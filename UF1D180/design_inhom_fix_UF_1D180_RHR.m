...
% Ver13 -- not relevant
% Ver14 -- Remove the assumptions that Zie=Zfa and Zfe=Zia. This will enable acquiring and exciting
%          in the same direction and not necessarily in opposite directions.
...
DNu0Zie     = DNu0(1);
DNu0Zfe     = DNu0(end);
dDNu0_dzZie = dDNu0_dz(1);
dDNu0_dzZfe = dDNu0_dz(end);
if (RevExcAcq)
	DNu0Zia     = DNu0(end);
	DNu0Zfa     = DNu0(1);
	dDNu0_dzZia = dDNu0_dz(end);
	dDNu0_dzZfa = dDNu0_dz(1);
else
	DNu0Zia     = DNu0(1);
	DNu0Zfa     = DNu0(end);
	dDNu0_dzZia = dDNu0_dz(1);
	dDNu0_dzZfa = dDNu0_dz(end);
end;
...
% Purge parameters
% ----------------
alpha2 = (1/(2*(Zie_-Zfe_))) * ( Tp * (2*gammaHz*Ge + dDNu0_dzZie + dDNu0_dzZfe) + ...
                                 (Ta/(Zfa-Zia)) * (dDNu0_dzZie*(Zie_-Zia) - ...
                                                   dDNu0_dzZfe*(Zfe_-Zia) + ...
                                                   DNu0Zfe                - ...
                                                   DNu0Zie) );                      % [1/cm^2]
%Ver12: alpha2 = (1/(2*(Zie_-Zia))) * ( (Ta/(Zfa-Zia)) * ( dDNu0_dz(1)*(Zie_-Zia) - DNu0(1) + DNu0(end) ) + ...
%                                Tp * (gammaHz*Ge + dDNu0_dz(1)) + ...
%                                gammaHz*Gpr*Tpr );                                 % [1/cm^2]
Gpr    = (1 / (gammaHz*Tpr)) * ( Tp * (gammaHz*Ge + dDNu0_dzZfe) - ...
                                 2*alpha2*(Zia-Zfe_)             - ...
                                 (Ta/(Zfa-Zia)) * (DNu0Zia - DNu0Zfe + dDNu0_dzZfe*(Zfe_-Zia)) );
%Ver12: Gpr    = (Tp / (gammaHz*Tpr)) * (gammaHz*Ge + dDNu0_dz(end));
alpha1 = -2*alpha2*Zia - (Ta/(Zfa-Zia))*DNu0Zia;                                    % [1/cm]
alpha0 = 0;                                                                         % [none]
% The following value is not used by the algorith, we calculate it just to see its value.
% The sign is added only for intuitive reasons, sue to the abs that is used.
