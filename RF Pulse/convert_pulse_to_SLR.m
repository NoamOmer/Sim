...
function [pulse,rfpwr,rfpwrf] = convert_pulse_to_SLR(z_axis,Tp,rfwdth,Phi_e_sq,OmegaE,OmegaE_axis,ze_of_OmegaE,...
                                                     swHz,isPI,tpwr90,pw90,slr_esp,DEBUG_FLAG,SIMULATION)
set_globals;
swkHz = swHz*1E-3;
...
if (SRkHz >= 0.9*swkHz)
	uiwait(msgbox(sprintf('SLR sampling rate might be too low...(SRkHz=%3.3f, swkHz=%3.3f)',SRkHz,swkHz)));
end;
freq_resp = window_freq_resp(swkHz,SRkHz,Tp*1E+3,slr_esp,(DEBUG_FLAG>2));
freq_resp = abs(freq_resp) + 1E-10;

if (0)
	% Modify frequency response so as not to excite the center sample region.
	% This will prevent artifacts from arriving from this region during the acquisition.
	% Do not reduce excitation to zero because we still wish to receive some signal from this region.
	% [noambe 07-oct-08:] This method is not used any longer. It produces
	% plausible improvement but not the as good as we wish.
...
freq_resp = freq_resp .* exp(1i*Phi_omega);
...
% Ugly - save the excitation profile in a file, to be used later
...
% Calibrate the pulse
cur_pwd = pwd;
cd 'E:\01 Post\Matlab\PulseCalibration';
