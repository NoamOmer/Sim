...
function st = set_evolve_M_n_acq_wTimeDepGrad_2D_C_struct(M_init,B_eff_rot,dB0_XY,Gx,Gy,dt,x_axis,y_axis,...
...
st.Gx          = Gx;                           % 8   vec
st.Gy          = Gy;                           % 9   vec
st.x_axis      = x_axis;                       % 10   vec
st.y_axis      = y_axis;                       % 11  vec
st.dt          = dt;                           % 12  scalar
st.acq_flag    = acq_flag;                     % 13  scalar
st.inhomo_flag = inhomo_flag;                  % 14  scalar
st.relax_flag  = relax_flag;                   % 15  scalar
st.plot_flag   = plot_flag;                    % 16  scalar
st.T1          = T1;                           % 17  scalar
st.T2          = T2;                           % 18  scalar
st.context     = context;                      % 19  string
