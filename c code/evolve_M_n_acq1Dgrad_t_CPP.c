...
#define acc_s_sz 10
	...
	// Inhomogeneity
	dB0z_in     =           mxGetPr(mxGetFieldByNumber(prhs[0],0,p_idx));   p_idx++;
	// z-Gradient
	zGrad_in    =           mxGetPr(mxGetFieldByNumber(prhs[0],0,p_idx));   p_idx++;
	// Spatial axis
	...
	if (!M_initx || !M_inity || !M_initz || !B_eff_rotx || !B_eff_roty || !B_eff_rotz || !dB0z_in || !zGrad_in || !z_axis) {
		mexPrintf("Error parsing params: %p,%p,%p,%p,%p,%p,%p,%p,%p",M_initx,M_inity,M_initz,B_eff_rotx,B_eff_roty,B_eff_rotz,dB0z_in,zGrad_in,z_axis);
...
//	acc_s_imag = mxGetPr(acc_s_mxArr[1]);    acc_s_imag[0] = 0;
	...
	/* -----------------------------
	    For faster run time ...
	   -----------------------------*/
	gamma_T_x_dt = gamma_T*dt;
	dB0z = (double *)calloc(n_r_steps,sizeof(double));
	for (r_idx = 0; r_idx < n_r_steps; r_idx++) {
		dB0z[r_idx]  = dB0z_in[r_idx]*inhomo_flag;
	}
	zGrad = (double *)calloc(n_t_steps,sizeof(double));
	for (t_idx = 0; t_idx < n_t_steps; t_idx++) {
		zGrad[t_idx] = zGrad_in[t_idx] * 1E-4;
	}


	/* -----------------------------
	    For faster run time some of this Bloch propagator functionalities were disabled. Validate that they are not needed...
	   -----------------------------*/
//	if (inhomo_flag) {
//		mexPrintf("Error: this variant of evolve_M_n_acq1Dgrad_t_CPP was modified not to support field inhomogeneity for faster run time");
//		mexErrMsgTxt("Exiting...");
//	}		
	if (!RH_flag) {
		mexPrintf("Error: this variant of evolve_M_n_acq1Dgrad_t_CPP was modified not to support LH_flag=1 for faster run time");
		mexErrMsgTxt("Exiting...");
	}		
	if (!relax_flag) {
		mexPrintf("Error: this variant of evolve_M_n_acq1Dgrad_t_CPP was modified not to support relax_flag=0 for faster run time");
		mexErrMsgTxt("Exiting...");
	}		
...
//			Bz = B_eff_rotz[t_idx] + B_grad_zt + dB0z[r_idx]*inhomo_flag;
			...
			{ // (3) even faster: comment all ... there's really nothing to be done here -- M_outx,y,z remain unchanged
				...
				// (2) faster: we know which elements of a unit R_mat are =1 and which are =0
				M_outx[r_idx] =               Mx;
				M_outy[r_idx] =                                  My;
				M_outz[r_idx] =                                                     Mz;

				// (1) original w/o rotation matrix
//				R_mat = unit_matrix(R_mat);                        // No magnetic field --> no precession
//				Mx = M_outx[r_idx];
//				My = M_outy[r_idx];
//				Mz = M_outz[r_idx];
				
                // (0) Initial w/ rotation matrix
...
//				M_outz[r_idx] = R_mat_unit[6]*Mx + R_mat_unit[7]*My + R_mat_unit[8]*Mz;
			...
			else
			{
				...
				Bnorm_y = By / B_vec_sz;
				Bnorm_z = Bz / B_vec_sz;
				phi_rot = gamma_T_x_dt*B_vec_sz;                     // [rad] = [rad/(sec*T)] * [T] * [sec]
// [fast]		if (RH_flag)
					R_mat = rot_RHR(Bnorm_x, Bnorm_y, Bnorm_z, phi_rot, R_mat);
// [fast]		else
// [fast]			R_mat = rot_LHR(Bnorm_x, Bnorm_y, Bnorm_z, phi_rot, R_mat);
