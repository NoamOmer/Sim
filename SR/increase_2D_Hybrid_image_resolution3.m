...
function [sig_mat_both_SR]=increase_2D_Hybrid_image_resolution3(sig_mat_both,Yie,Yfe,Ly,initial_dy,required_dy,...
                                                    alpha0,alpha1,alpha2,Tp,NSE,TaPE,Ta,GaPE,SE_flag,SRExp,SRESP,...
                                                    SR_ones_win_flag,SRnIter,SR_zero_init_guess,SR_fix_inhomo_flag,...
													SR_cs,ax_spect,T2PE_factor)
set_globals;
...
if (SR_fix_inhomo_flag)
	...
	if (DEBUG_FLAG >= 2), figure; imagesc(map_mat_HR); title('High-res map mat'); set(gca,'YDir','normal'); end; % caxis([-300 300]);
...
if (DEBUG_FLAG >= 3)
figure;
subplot(1,2,1); imagesc(ones(M,1)*phi_e_HR + transpose(k)*HR_y_axis); title('A [rad]');
subplot(1,2,2); imagesc(W_f);                                         title('W f [normalized]');
% figure;
...
check_idx = 90; %round(size(sig_mat_both,1)/2);
...
% 		dB0_HR = polyval(a_HR,y_HR);
		...
		dPh_B0e    = 2*pi * (transpose(Tp-te) * Phi_e_sign*dB0);             % [rad]
		dPh_B0e_HR = 2*pi * (transpose(Tp-te) * Phi_e_sign*dB0_HR);
		dPh_B0a    = 2*pi * (transpose(Ta - ta) * dB0);                      % [rad]
		dPh_B0a_HR = 2*pi * (transpose(Ta - ta) * dB0_HR);
...
% % 		dB0_W_f_0 = dB0_W_f_0 .^ 15;
% % 		dB0_W_f_0 = flipdim(dB0_W_f_0,1);
...
% 			figure; imagesc(dB0_W_f_0); title('dB0 W f 0'); colorbar;
			figure;
			subplot(2,2,1); plot(y_axis,dB0,'.-'); title('dB0    ');
			subplot(2,2,2); imagesc(dPh_B0e);      title('dPh B0e');                      colorbar;
			subplot(2,2,3); imagesc(dPh_B0a);      title('dPh B0a');                      colorbar;
			subplot(2,2,4); imagesc(dPh_B0);       title('dPh B0e + dPh B0a');            colorbar;
			figure;
			subplot(2,2,1); imagesc(dPh_B0_HR           ); title('dPh B0 HR');            colorbar;
			subplot(2,2,2); imagesc(Ph_A0_HR            ); title('Ph A0 HR');             colorbar;
			subplot(2,2,3); imagesc(dPh_B0_HR + Ph_A0_HR); title('dPh B0 HR + Ph A0 HR'); colorbar;
			subplot(2,2,4); imagesc(dB0_W_f             ); title('dB0 W f');              colorbar;
		end;
	end;
	...
	if (DEBUG_FLAG >= 2) && plot_f && (SR_fix_inhomo_flag)
		figure; subplot(2,2,1); plot(dB0,'.-');         title('dB0');
				subplot(2,2,2); plot(f0,'.-');          title('fo');
		figure; subplot(3,2,3); imagesc(dPh_B0);        title('dPh B0');       set(gca,'ydir','normal'); colorbar;
		        subplot(3,2,1); imagesc(dPh_B0e);       title('dPh B0e');      set(gca,'ydir','normal'); colorbar;
		        subplot(3,2,2); imagesc(dPh_B0a);       title('dPh B0a');      set(gca,'ydir','normal'); colorbar;
		        subplot(3,2,4); imagesc(Ph_A0);         title('Ph A0');        set(gca,'ydir','normal'); colorbar;
		        subplot(3,2,5); imagesc(Ph_A0+dPh_B0);  title('Ph A0 + B0');   set(gca,'ydir','normal'); colorbar;
% 		figure; subplot(2,2,1); imagesc(angle(A0));     title('Angle A0');     set(gca,'ydir','normal');
% 		        subplot(2,2,2); imagesc(angle(A_dB0));  title('Angle A dB0');  set(gca,'ydir','normal');
% 				subplot(2,2,3); imagesc(angle(A));      title('Angle A');      set(gca,'ydir','normal');
	end;
...
if (DEBUG_FLAG >= 2)
figure;
subplot(2,2,1);imagesc(imgX,imgX,transpose(abs(sig_mat)));title('Image w/  axes');set(gca,'YDir','normal');xlabel('RO-axis [cm]');ylabel('PE-axis [cm]');axis image;
subplot(2,2,2);imagesc(mapX,mapY(end:-1:1),transpose(map_mat));     title('Map   w/  axes');set(gca,'YDir','normal');xlabel('RO-axis [cm]');ylabel('PE-axis [cm]');axis image;%caxis([-300 300]);
...
if (DEBUG_FLAG >= 3)
% subplot(2,2,3);
figure;imagesc(map_mat); title('Low-res map mat'); set(gca,'YDir','normal'); % caxis([-300 300]);
end;
return;
