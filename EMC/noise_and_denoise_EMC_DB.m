% 2018_08_06   |   Neta Stern   |   Added rng reset flag. Default behavior: RNG seed is NOT reset
function [emc_db_wNoise,emc_db_DeNoised] = noise_and_denoise_EMC_DB(snr,emc_db,denoise_opts,inETL,plot_f,rst_flag)
...
if (exist('rst_flag','var')) && (rst_flag)
...
for B1_idx = 1:nB1
	...
	for T1_idx = 1:nT1s
	for T2_idx = 1:nT2s
		emc_vec  = transpose(squeeze(emc_db_T1T2(1,1,T1_idx,T2_idx,:))); % [NS]
