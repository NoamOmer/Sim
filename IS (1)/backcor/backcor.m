...
hwin = figure('Visible','on','Position',[0 0 750 400],'NumberTitle','off','Name','Background Correction',...
    'MenuBar','none','Toolbar','figure','Resize','on','ResizeFcn',{@WinResizeFcn});
...
% Callback functions
    ...
    function WinResizeFcn(source,eventdata)
        % Resize the window
% 		defaultFigureUnits : [ inches | centimeters | normalized | points | {pixels} | characters ]
% 		set(0,'defaultfigureunits', 'points');	
        ...
        h = pos(4);
        if w>400 && h>100,
            set(haxes,'Position',[40/FF,40/FF,w-320/FF,h-70/FF]);
        end;
        set(hok,'Position',[w-90/FF,30/FF,80/FF,25/FF]);
        set(hcancel,'Position',[w-180/FF,30/FF,80/FF,25/FF]);
        set(hfctlbl,'Position',[w-240/FF,h-30/FF,220/FF,20/FF]);
        set(hfct,'Position',[w-240/FF,h-50/FF,220/FF,25/FF]);
        set(hthresholdlbl,'Position',[w-240/FF,h-80/FF,220/FF,20/FF]);
        set(hthreshold,'Position',[w-240/FF,h-100/FF,220/FF,20/FF]);
        set(horderlbl,'Position',[w-240/FF,h-130/FF,220/FF,20/FF]);
        set(horder,'Position',[w-210/FF,h-150/FF,190/FF,20/FF]);
        set(horderval,'Position',[w-240/FF,h-150/FF,20/FF,20/FF]);
    end
